#!/usr/bin/env bash
# Focused single-turn SGLang sweep for Jetson AGX Thor. Mirrors sweep_thor_vllm.sh:
# Thor SGLang image, JETSON_PLATFORM env, MAXN locked clocks, identical CSV schema, resumable.
#
# L-shape grid (~15 cells, not full 6x6 -> saves hours; covers all paper figures):
#   - fp16 (HF safetensors) ISL sweep @ gen=128   -> Fig5 prefill TTFT vs ISL
#   - fp16 (HF safetensors) OSL sweep @ pp=128     -> Fig6 decode TPOT vs OSL
#   - fp16 4Kx4K corner                            -> Fig7/8 energy (decode dominates)
#   - GGUF Q8_0 / Q4_K_M / Q4_0 @ 128x128          -> Fig9 / Table4 quant ladder (best-effort;
#                                                     SGLang GGUF path may skip if unsupported)
#
# Usage:  sg docker -c 'cd data/benchmarks && bash sweep_thor_sglang.sh'
set -uo pipefail
...
SGLANG_IMAGE="${SGLANG_IMAGE:-thor:r38.3.arm64-sbsa-cu130-24.04-sglang_0.5.7}"
...
OUTPUT_CSV="${OUTPUT_CSV:-$OUTDIR/sweep_thor_sglang_$(date +%Y%m%d_%H%M%S).csv}"
...
# Thor SGLang env caveats:
#  - FLASHINFER_DISABLE_VERSION_CHECK=1: sglang[all] pulled flashinfer-cubin 0.6.8 over our built
#    flashinfer 0.5.4; bench uses triton attn (Llama is dense, no MoE) so cubins aren't on our path.
#  - CPATH: triton 3.6.0 JIT needs cuda.h but its bundled backends/nvidia/include is absent on this
#    build; point gcc at the system CUDA 13 headers so the triton driver module compiles.
...
run_exists() { grep -q ",sglang,Llama-3.2-1B,${1},${2},${3}," "$OUTPUT_CSV" 2>/dev/null; }  # quant,pp,gen
...
print(','.join(str(x) for x in [
 ts,'sglang','Llama-3.2-1B',quant,pp,gen,d.get('generated_tokens',gen),
...
echo "===== SGLang Thor sweep -> $OUTPUT_CSV ====="
echo "--- fp16 ISL sweep @ gen=128 (Fig5 prefill) ---"
for pp in "${LENGTHS[@]}"; do run_cell fp16 "$FP16_MODEL" "$FP16_MOUNT" "$pp" 128; done
echo "--- fp16 OSL sweep @ pp=128 (Fig6 decode) ---"
for gen in 256 512 1024 2048 4096; do run_cell fp16 "$FP16_MODEL" "$FP16_MOUNT" 128 "$gen"; done
echo "--- fp16 4Kx4K corner (Fig7/8 energy) ---"
run_cell fp16 "$FP16_MODEL" "$FP16_MOUNT" 4096 4096
echo "--- GGUF quant ladder @128x128 (best-effort) ---"
for q in Q8_0 Q4_K_M Q4_0; do
    run_cell "gguf_$q" "/models/gguf/Llama-3.2-1B-Instruct-$q.gguf" "-v $DATA_DIR/models:/models" 128 128
done
