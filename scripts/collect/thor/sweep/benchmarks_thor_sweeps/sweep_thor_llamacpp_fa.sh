#!/usr/bin/env bash
# llama.cpp + FlashAttention sweep for Jetson AGX Thor (framework="llamacpp_fa").
...
OUTPUT_CSV="${OUTPUT_CSV:-$OUTDIR/sweep_thor_llamacpp_fa_$(date +%Y%m%d_%H%M%S).csv}"
FLASH_ATTN=1
...
run_exists() { grep -q ",llamacpp_fa,Llama-3.2-1B,${1},${2},${3}," "$OUTPUT_CSV" 2>/dev/null; }
...
print(','.join(str(x) for x in [
 ts,'llamacpp_fa','Llama-3.2-1B',quant,pp,gen,d.get('generated_tokens',gen),
