...
int main(int argc, char **argv) {
  ...
  // theoretical peak from device props (2 = DDR), for cross-check with --peak
  double prop_peak = 2.0 * prop.memoryClockRate * 1e3 /*kHz->Hz*/ *
  ...
  if (!csv) {
    ...
    fprintf(stderr, "# memClock=%.0f MHz busWidth=%d bits -> prop peak=%.1f GB/s\n",
            prop.memoryClockRate / 1e3, prop.memoryBusWidth, prop_peak);
