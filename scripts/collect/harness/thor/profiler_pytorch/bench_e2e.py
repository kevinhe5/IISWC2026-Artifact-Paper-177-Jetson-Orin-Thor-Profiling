...
def load_model(model_id: str, quantization: str = "4bit", num_layers: int = None, context_size: int = None, power_tracer=None):
    ...
    config_kwargs = {}
    # Attention impl: paper/Orin "no-FA" config = true eager attention. transformers 5.x
    # defaults to SDPA which dispatches to FlashAttention on Thor (artificially fast).
    # Force eager to match the paper; override via PYTORCH_ATTN_IMPL.
    config_kwargs["attn_implementation"] = os.environ.get("PYTORCH_ATTN_IMPL", "eager")
    if num_layers:
...
def run_e2e_benchmark(
    ...
    # Calculate averages. Drop the FIRST run as warmup when >=2 runs: it absorbs the
    # one-time torch.compile / Inductor compilation cost (catastrophic on Thor sm_110:
    # run1 decode ~394ms vs steady-state ~10ms). The paper's TPOT is steady-state per-token
    # (compile amortized), so excluding the compile run is what matches it. Single-run keeps all.
    if len(prefill_times) >= 2:
        _pf, _dc, _tt, _tg = prefill_times[1:], decode_times[1:], total_times[1:], tokens_generated_list[1:]
        print(f"  [avg] dropping run1 as warmup ({len(_pf)} timed runs)", file=sys.stderr)
    else:
        _pf, _dc, _tt, _tg = prefill_times, decode_times, total_times, tokens_generated_list
    avg_prefill = sum(_pf) / len(_pf)
    avg_decode = sum(_dc) / len(_dc)
    avg_total = sum(_tt) / len(_tt)
    avg_tokens_gen = sum(_tg) / len(_tg)
