...
def run_e2e_benchmark(model_path: str, prompt_tokens: int, gen_tokens: int, num_runs: int) -> dict:
    ...
    _eager = os.environ.get("VLLM_ENFORCE_EAGER", "1") == "1"
    llm_kwargs = dict(model=model_path, dtype="float16", max_model_len=max_len,
                      enforce_eager=_eager, gpu_memory_utilization=gpu_mem_util,
                      trust_remote_code=True)
    if disable_cache:
        llm_kwargs["enable_prefix_caching"] = False
    # async_scheduling overlaps CPU step-prep with GPU exec (V1 engine speedup).
    # VLLM_ASYNC_SCHEDULING=0 serializes them, restoring the per-step launch gap
    # of the paper-era engine. Default: leave at vLLM's default (unset).
    _async = os.environ.get("VLLM_ASYNC_SCHEDULING")
    if _async is not None:
        llm_kwargs["async_scheduling"] = (_async == "1")
    llm = LLM(**llm_kwargs)
